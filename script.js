...
try {
  const s = localStorage.getItem('ai_hub_favs');
...
// ─── FAVORITES (LocalStorage — persists on refresh) ──────
function updateStars() {
  document.querySelectorAll('.tool-row').forEach(row => {
    const btn = row.querySelector('.fav-btn');
    const isFav = favs.includes(row.dataset.id);
    btn.classList.toggle('active', isFav);
    btn.title = isFav ? 'Remove from Saved' : 'Save this tool';
  });
}
